...
export async function updateDeliveryStatus(deliveryId, status) {
  try {
    const response = await fetch(
      `http://localhost:5003/api/deliveries/${deliveryId}/status`,
      {
        method: "PUT",
        headers: {
...
export async function getAvailableDeliveries() {
  try {
    const response = await fetch(
      `http://localhost:5003/api/deliveries/available`,
      {
        credentials: "include",
      }
    );
...
// Add a function to get orders ready for pickup
export async function getOrdersReadyForPickup() {
  try {
    const response = await fetch(
      `http://localhost:5003/api/deliveries/orders/ready-for-pickup`,
      {
        credentials: "include",
      }
    );
    ...
    if (!response.ok) {
      throw new Error("Failed to fetch orders ready for pickup");
    }
