...
import { ArrowLeft, Search, Clock, MapPin, ChevronRight, ShoppingBag, DollarSign, Info } from "lucide-react"
...
export default function UserOrdersPage() {
        ...
        if (query) {
            ...
            filtered = filtered.filter(
                (order) =>
                    ...
                    (order.restaurant?.name && order.restaurant.name.toLowerCase().includes(lowercaseQuery)),
            )
        }
    ...
    const getStatusBadge = (status) => {
        const statusMap = {
            ...
            READY_FOR_PICKUP: { color: "bg-indigo-100 text-indigo-800", label: "Ready for Pickup" },
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-100 p-6">
            <div className="max-w-4xl mx-auto">
                <Button variant="ghost" className="mb-6" onClick={() => router.push("/dashboard/user")}>
                            ...
                            <TabsContent value={activeTab} className="mt-0">
                                {filteredOrders.length === 0 ? (
                                    <div className="text-center py-12">
                                        ...
                                        <Button onClick={() => router.push("/dashboard/user/restaurants")}>Browse Restaurants</Button>
                                    </div>
                                ) : (
                                    <div className="space-y-4">
                                        {filteredOrders.map((order) => (
                                            <div
                                                key={order._id}
                                                className={`bg-white rounded-lg shadow-sm p-4 hover:shadow-md transition-shadow ${order.is_demo ? "border-l-4 border-amber-300" : ""
                                                    }`}
                                            >
                                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                                    <div className="flex-1">
                                                        ...
                                                        <p className="text-sm text-gray-500 mb-2">{order.restaurant?.name || "Restaurant Name"}</p>
