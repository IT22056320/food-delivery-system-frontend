...
export default function RestaurantDetailPage() {
  ...
  const [favoritedMenuItems, setFavoritedMenuItems] = useState({});
  ...
  useEffect(() => {
    if (restaurant) {
      setIsFavorited(isFavorite("restaurant", restaurant._id));
    }
  ...
  useEffect(() => {
    if (menuItems.length > 0) {
      ...
      menuItems.forEach((item) => {
        favoritedItems[item._id] = isFavorite("menuItem", item._id);
      });
      setFavoritedMenuItems(favoritedItems);
    }
  ...
  const toggleFavorite = () => {
    ...
    if (isFavorited) {
      removeFromFavorites("restaurant", restaurant._id);
  ...
  const toggleMenuItemFavorite = (item, event) => {
    event.stopPropagation(); // Prevent clicking the menu item itself

    const isFavorited = favoritedMenuItems[item._id];

    if (isFavorited) {
      removeFromFavorites("menuItem", item._id);
      setFavoritedMenuItems((prev) => ({
        ...prev,
        [item._id]: false,
      }));
                            ...
                            >
                              ...
                              <div className="flex gap-2">
                                <Button
                                  ...
                                  size="icon"
                                  className={`rounded-full ${
                                    favoritedMenuItems[item._id]
                                      ? "text-red-500 hover:text-red-600"
                                      : "text-gray-400 hover:text-gray-500"
                                  }`}
                                  onClick={(e) =>
                                    toggleMenuItemFavorite(item, e)
                                  }
                                >
                                  <Heart
                                    className={`h-4 w-4 ${
