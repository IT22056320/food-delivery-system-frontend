...
export default function RestaurantDashboard() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        ...
        // Normalize order status formats from different services
        const normalizedPendingOrders = pending.map((order) => {
          // Check if this is the new format (with order_status) or old format (with status)
          ...
          return {
            ...order,
            status: status,
            // Add a unique service prefix to the ID to prevent duplicate key errors
            _displayId: order.customer_id
              ? `order-${order._id}`
              : `rest-${order._id}`,
          };
        });

        const normalizedCompletedOrders = completed.map((order) => {
          const status = order.order_status
            ? order.order_status.toLowerCase()
            : order.status;
          return {
  ...
  const getCompletionRate = () => {
    ...
    if (total === 0) return 0;
    const delivered = completedOrders.filter((order) => {
      const status = order.status || order.order_status?.toLowerCase();
                ...
                <div className="flex flex-col items-center gap-2">
                  <div className="flex items-center justify-between w-full mb-2">
                    ...
                    <Switch
                      ...
                      onCheckedChange={() =>
                        handleToggleAvailability(selectedRestaurant)
                      }
                      ...
                      <tbody>
                        {pendingOrders.slice(0, 2).map((order, index) => (
                          ...
                          >
                            ...
                            <td className="py-3">
                              $
                              {(order.totalAmount || order.total_price).toFixed(
                                2
                              )}
                            </td>
                            <td className="py-3">
                            ...
                            <td className="py-3">
                              <div className="flex items-center gap-2">
                                ...
                                <Button
                                  ...
                                  onClick={() =>
                                    router.push(
                                      `/dashboard/restaurant/orders/${order._id}`
                                    )
                                  }
                                >
                ...
                <CardHeader className="pb-2">
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <CardDescription>
                        Overview of {selectedRestaurant.name}'s performance
                      </CardDescription>
                    </div>
                    <Button
                      variant="outline"
                      ...
                      onClick={() =>
                        router.push("/dashboard/restaurant/performance")
                      }
                    >
            ...
            <Card className="border-none shadow-md bg-orange-500 text-white">
              ...
              <CardContent className="space-y-3">
                <Button
                  ...
                  onClick={() =>
                    router.push("/dashboard/restaurant/menu/create")
                  }
                >
                ...
                <Button
                  ...
                  onClick={() =>
                    selectedRestaurant &&
                    router.push(
                      `/dashboard/restaurant/edit/${selectedRestaurant._id}`
                    )
                  }
